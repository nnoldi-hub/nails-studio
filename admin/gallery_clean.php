...
if ($_POST) {
    if (isset($_POST['add_gallery'])) {
        ...
        $debug_info = "=== UPLOAD ATTEMPT ===\n";
        $debug_info .= "POST DATA: " . print_r($_POST, true) . "\n";
        $debug_info .= "FILES DATA: " . print_r($_FILES, true) . "\n";
        file_put_contents('../debug_upload.log', date('Y-m-d H:i:s') . " " . $debug_info, FILE_APPEND);
        
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            $upload_dir = '../assets/images/gallery/';
            if (!is_dir($upload_dir)) {
                mkdir($upload_dir, 0755, true);
                file_put_contents('../debug_upload.log', "Created directory: $upload_dir\n", FILE_APPEND);
            }
            ...
            file_put_contents('../debug_upload.log', "Original name: $original_name\n", FILE_APPEND);
            file_put_contents('../debug_upload.log', "File size: $file_size bytes\n", FILE_APPEND);
            ...
            if (move_uploaded_file($tmp_name, $target_path)) {
                file_put_contents('../debug_upload.log', "SUCCESS: File moved to $target_path\n", FILE_APPEND);
                ...
                if ($stmt->execute()) {
                    ...
                    file_put_contents('../debug_upload.log', "SUCCESS: Database insert successful!\n", FILE_APPEND);
                } else {
                    $error_message = "Eroare la salvarea în baza de date: " . $conn->error;
                    file_put_contents('../debug_upload.log', "ERROR: Database insert failed: " . $conn->error . "\n", FILE_APPEND);
                ...
                file_put_contents('../debug_upload.log', "ERROR: move_uploaded_file FAILED!\n", FILE_APPEND);
            }
        } else {
            if (!isset($_FILES['image'])) {
                $error_message = "Nu a fost selectat niciun fișier.";
                file_put_contents('../debug_upload.log', "ERROR: No file in _FILES array\n", FILE_APPEND);
            } else {
                $error_code = $_FILES['image']['error'];
                $error_message = "Eroare la upload (cod: $error_code)";
                file_put_contents('../debug_upload.log', "ERROR: Upload error code: $error_code\n", FILE_APPEND);
            }
        }
        
        file_put_contents('../debug_upload.log', "=== END UPLOAD ATTEMPT ===\n\n", FILE_APPEND);
    }
    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            ...
            <div class="alert alert-info">
                ...
                Upload temp dir: <?php echo ini_get('upload_tmp_dir'); ?>
            </div>
