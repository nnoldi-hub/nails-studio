...
require_admin_login();
$page_title = 'Raport Programări Cursuri';
...
// Query programări cursuri filtrate
$where = [];
$params = [];
$types = '';
if ($start_date) {
...
// Adaugă prețul cursului la query
$sql = 'SELECT cb.*, cs.session_name, cs.price AS session_price FROM coaching_bookings cb JOIN coaching_sessions cs ON cb.session_id = cs.id';
if ($where) {
    $sql .= ' WHERE ' . implode(' AND ', $where);
}
$sql .= ' ORDER BY cb.booking_date DESC, cb.booking_time DESC';
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    header('Content-Disposition: attachment; filename=raport_cursuri.csv');
    $output = fopen('php://output', 'w');
    fputcsv($output, ['ID', 'Client', 'Email', 'Telefon', 'Curs', 'Data', 'Ora', 'Status']);
    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2">Raport Programări Cursuri</h1>
                <div>
            ...
            <div class="card shadow">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-primary">Rezervări Filtrate</h6>
                </div>
                <div class="card-body">
                    <div id="printArea">
                            ...
                            <table class="table table-bordered table-hover" style="margin:0 auto;">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($booking['session_name']); ?></td>
                                        <td>
                                            <?php 
                                            if (isset($booking['session_price']) && $booking['session_price'] !== null) {
                                                $total_pret += (float)$booking['session_price'];
                                                echo number_format((float)$booking['session_price'], 0) . ' RON';
                                            } else {
                                                echo 'N/A';
                                            }
                                            ?>
                        ...
                        <div class="text-end mt-3">
                            <strong>Total preț rezervări filtrate: <?php echo number_format($total_pret, 0); ?> RON</strong>
                        </div>
                            </table>
                        </div>
                        <?php else: ?>
                        <div class="text-center py-4">
                            ...
                            <h5>Nu există rezervări pentru filtrul selectat</h5>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
